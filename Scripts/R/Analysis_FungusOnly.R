#29th Janaury 2023
#Analysis : Fungal Count Data
#Prokaryotes
#See Prokaryote Script for Resource details
set.seed(1122)

dir.create("output/Fungal/")
dir.create("output/Fungal/Composition_Analysis/")
...
#count table
cnt <- read.table("input/Counts/Fungus_RawCounts.tsv")
...
write.table(cnt, "input/Counts/Fungus_Filtered_Raw.tsv",
...
#write up
write.table(cnt.rel, "input/Counts/Fungus_RelativeAbundance.tsv",
            col.names = T, row.names = T, quote = F)
...
write.table(cnt, "input/Counts/Fungus_Filtered_FamReduced_Raw.tsv",
...
#write up
write.table(cnt.lab,
            "input/Counts/Fungus_Filtered_TribeReduced_Raw_MicroTaxaLabelled.tsv",
            quote = F, sep = "\t", row.names = T, col.names = T)
write.table(cnt.rel.lab,
            "input/Counts/Fungus_Filtered_TribeReduced_RelAb_MicroTaxaLabelled.tsv",
            quote = F, sep = "\t", row.names = T, col.names = T)
...
sum((as.vector(pca$CA$eig)/sum(pca$CA$eig))[1:2]) # 45.6%
sum((as.vector(pca$CA$eig)/sum(pca$CA$eig))[1:4]) # 65.8%
...
for (i in 1:length(vars)){
  ...
  ggsave(paste("output/Fungal/Composition_Analysis/PCA_PC1PC2_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = propc.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Fungal/Composition_Analysis/PCA_PC3PC4_", title, ".pdf", sep = ""))
}

#Looking at three axes at once
sum((as.vector(pca$CA$eig)/sum(pca$CA$eig))[1:3]) # 57.0%
...
for (i in 1:length(vars)){
  ...
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Fungal/Composition_Analysis/PCA_3D_", title, ".pdf", sep = ""))
}

##Step Three: BiPlot ####
biplot(pca, choices = c(1,2), type = c("text", "points"))

pdf("output/Fungal/Composition_Analysis/BiPlot_PC1PC2.pdf")
biplot(pca, choices = c(1,2), type = c("text", "points"))
dev.off()
#without text
pdf("output/Fungal/Composition_Analysis/BiPlot_PC1PC2_Plain.pdf")
biplot(pca, choices = c(1,2), type = c("points"))
dev.off()

#there's three clear taxa that affect these ordinations (plus a possible fourth)
vip <- c("TRH13", "TRH23", "TRH462")
...
#The following sampling units were removed because they were below sampling depth: CA_N_007, CA_N_030, SRR11426433, SRR12053426, SRR3948521, SRR3948533, SRR3948536, SRR3948544, SRR3948566, SRR3948570, SRR3948575, SRR6148372, SRR8754044, SRR8867394
 

#write up
d <- as.matrix(dist)
write.table(d, "output/Fungal/Composition_Analysis/BC_DistanceMatrix.tsv",
...
for (i in 1:length(vars)){
  ggplot(data = pco.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Fungal/Composition_Analysis/PCoA_Ax1Ax2_", title, ".pdf", sep = ""))
}

##Step Five: NonMetric Multidimensional Scaling ####
pro.scree <- scree.NMDS(dist, "Fungal", "Fungal")
pro.scree
...
#save
pdf("output/Fungal/Composition_Analysis/NMDS_StressPlot.pdf")
stressplot(nmds)
dev.off()
...
#save
write.table(nmds.plot, "output/Fungal/Composition_Analysis/NMDS_meta.tsv",
...
for (i in 1:length(vars)){
  ggplot(data = nmds.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Fungal/Composition_Analysis/NMDS_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = nmds.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Fungal/Composition_Analysis/NMDS_Ellipses_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = nmds.plot, 
    ...
    guides(fill = "none")
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Fungal/Composition_Analysis/NMDS_Polygons_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ...
  if(any(table(metdf.pd[,vars[i]]) < 4)){
    ...
    ggplot(data = nmds.plot, 
      ...
      theme(strip.background = element_blank())
    title <- gsub(" ", "_", varslabs[i])
    ggsave(paste("output/Fungal/Composition_Analysis/NMDS_", title, 
...
for (i in 1:length(foi)){
  ...
  ggsave(paste("output/Fungal/Composition_Analysis/NMDS_3D_", 
               title, ".pdf", sep = ""))
}
...
#write up
write.table(disp.df, "output/Fungal/Composition_Analysis/VariableDispersion_ANOVA.tsv",
...
#nothing is significant that isn't affected by heterogeneous dispersion
multiSIM

#write up
write.table(multiSIM, "output/Fungal/Composition_Analysis/Multi_ANOSIM_testResults.tsv",
...
#no sig difference between any socialities

write.table(pairSIM.soc, 
            "output/Fungal/Composition_Analysis/PairwiseANOSIM_Sociality.tsv",
            col.names = T, row.names = F, quote = F,
            sep = "\t")

#pairwise: Continent
cbn <- combn(x = unique(metdf.pd$Continent), m = 2)
pvalue <- c()

for (i in 1:ncol(cbn)){
  sub <- metdf.pd$Sample.ID[metdf.pd$Continent == cbn[,i]]
  subcnt <- cnt[names(cnt) %in% sub]
  submet <- metdf.pd[metdf.pd$Sample.ID %in% names(subcnt),]
  subdist <- avgdist(t(subcnt), dmethod = "bray", sample = low_read, iterations = 1e4)
...
#this just straight up failed

#write.table(pairSIM.con, "output/Fungal/Composition_Analysis/PairwiseANOSIM_Continent.tsv",
 #           col.names = T, row.names = F, quote = F,
  #          sep = "\t")

#pairwise: Family
cbn <- combn(x = unique(metdf.pd$Family), m = 2)
pvalue <- c()
...
#no difference in families

write.table(pairSIM.fam, 
            "output/Fungal/Composition_Analysis/PairwiseANOSIM_Continent.tsv",
            col.names = T, row.names = F, quote = F,
            sep = "\t")
...
#except for year collected library layout and host family/tribe
...
#nothing is significant that isn't heterogenously dispersed
#write up
write.table(multiAOV, "output/Fungal/Composition_Analysis/Multi_adonis2_results.tsv",
            row.names = F, col.names = T, quote = F,
            sep = "\t")
...
pairwiseAOV.soc %>%
  filter(padj < 0.05)
#no difference between indivdual socialities
...
pairwiseAOV.fam %>%
  filter(padj < 0.05)
#no sig differences between different families

write.table(pairwiseAOV.fam, 
            "output/Fungal/Composition_Analysis/PairwiseAOV_Family.tsv",
            row.names = F, col.names = T, quote = F, sep = "\t")
...
pairwiseAOV.con %>%
  filter(padj < 0.05)
#doesn't work ? no idea why haven't got time to pursue.

#write.table(pairwiseAOV.con, 
 #           "output/Fungal/Composition_Analysis/PairwiseAOV_Continent.tsv",
  #          row.names = F, col.names = T, quote = F, sep = "\t")

###Modellling #####

#social, family, continent
p <- 9999
...
soc <- adonis2(dist ~ Sociality,
                   ...
                   permutations = p)
soc
#sociality is positive and nonsig dispersed
#this makes no sense from looking at the nmds plots I dunny trust it
#also nothing is sig in pairwise anova ..... dunno what's happening here
#r2 is also like 3% ... so not great like.
...
writeLines(capture.output(sessionInfo()),
           "SessionLogs/Analysis_Fungal_Jan23.txt")
