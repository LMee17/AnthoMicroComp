...
# over 20% of variance explained by first PC (would probably expect a good pca to be up
# to 60 but there is so much noise in this dataset)

# Calculate the percent of variance explained by first two to four axes
sum((as.vector(pca$CA$eig)/sum(pca$CA$eig))[1:2]) # 32.8%
sum((as.vector(pca$CA$eig)/sum(pca$CA$eig))[1:4]) # 53.8%
...
for (i in 1:length(vars)){
  ...
  ggsave(paste("output/Eukaryote/Composition_Analysis/PCA_PC1PC2_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = propc.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Eukaryote/Composition_Analysis/PCA_PC3PC4_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ...
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Eukaryote/Composition_Analysis/PCA_3D_", 
               title, ".pdf", sep = ""))
}
...
pdf("output/Eukaryote/Composition_Analysis/BiPlot_PC1PC2.pdf")
  biplot(pca, choices = c(1,2), type = c("text", "points"))
dev.off()
#without text
pdf("output/Eukaryote/Composition_Analysis/BiPlot_PC1PC2_Plain.pdf")
  biplot(pca, choices = c(1,2), type = c("points"))
dev.off()

#there's three clear taxa that affect these ordinations (plus a possible fourth)
vip <- c("TRH23", "TRH101", "TRH462")
taxkey[taxkey$taxID %in% vip,]
#Nosema (yay), Leptomonas (a tryp) and Aspergillus 
...
#again, first eigenvalue explains over 20% of the variance
...
for (i in 1:length(vars)){
  ggplot(data = pco.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Eukaryote/Composition_Analysis/PCoA_Ax1Ax2_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = nmds.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Eukaryote/Composition_Analysis/NMDS_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = nmds.plot, 
    ...
    theme(strip.background = element_blank())
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Eukaryote/Composition_Analysis/NMDS_Ellipses_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ggplot(data = nmds.plot, 
    ...
    guides(fill = "none")
  title <- gsub(" ", "_", varslabs[i])
  ggsave(paste("output/Eukaryote/Composition_Analysis/NMDS_Polygons_", title, ".pdf", sep = ""))
}
...
for (i in 1:length(vars)){
  ...
  if(any(table(metdf.pd[,vars[i]]) < 4)){
    ...
    ggplot(data = nmds.plot, 
      ...
      theme(strip.background = element_blank())
    title <- gsub(" ", "_", varslabs[i])
    ggsave(paste("output/Eukaryote/Composition_Analysis/NMDS_", title, 
...
for (i in 1:length(foi)){
  ...
  ggsave(paste("output/Eukaryote/Composition_Analysis/NMDS_3D_", 
               title, ".pdf", sep = ""))
}
...
#write up
write.table(multiSIM, 
            "output/Eukaryote/Composition_Analysis/Multi_ANOSIM_testResults.tsv",
...
write.table(pairSIM.soc, 
            "output/Eukaryote/Composition_Analysis/PairwiseANOSIM_Sociality.tsv",
...
#No difference between any of the continents (closest is between North America
#and Asia)

write.table(pairSIM.con, 
            "output/Eukaryote/Composition_Analysis/PairwiseANOSIM_Continent.tsv",
            col.names = T, row.names = F, quote = F,
            sep = "\t")
...
#there's no sig differences between family
...
#sociality is significant, tribe is not
...
soc.fam <- adonis2(dist ~ Sociality + Family,
                   ...
                   permutations = p)
soc.fam
#sociality is significant, family is not
...
soc.fam.nest <- adonis2(dist ~ Family / Sociality,
                        ...
                        strata = metdf$Family)
soc.fam.nest
#sociality as nested within familly continues to be significant

#throwing all phylo in ...
#this isn't necessary and is likely incorrect: just stick to family
...
soc.con <- adonis2(dist ~ Sociality + Continent,
                   ...
                   permutations = p)
soc.con
#both sociality and continent come back as significant
...
soc.con.nest
#both continent and sociality nested in continent are significant

soc.con.int <- adonis2(dist ~ Sociality*Continent,
                       data = metdf,
                       permutations = p)
soc.con.int
#Sociality is sig, continent is sig, not as an interaction
...
soc.yea <- adonis2(dist ~ Sociality + YearCollected,
                   ...
                   permutations = p)
soc.yea
#sociality and year collected is sig

adonis2(dist ~ Sociality*YearCollected,
        data = metdf,
        permutations = p)
#Sociality and Year collected is sig, interaction not
...
soc.all <- adonis2(dist ~ Sociality + Family/Tribe + YearCollected + Continent,
                   ...
                   strata = metdf$Family)
soc.all
#sociality, continent, and year collected are sig: not phylogeny
...
eu.stress <- scree.NMDS(eu.dist, "Eukaryote", "Eusocial")

if(any(eu.stress$Dec == "Excellent") == T){
  kval <- eu.stress %>%
    subset(Dec == "Excellent") %>%
    arrange(NoDimensions) %>%
    head(n = 1) %>%
  ...
  print("Please assess by eye")
  stopifnot(any(eu.stress$Dec == "Excellent"))
}

eu.nmds <- metaMDS(dist, k = kval, trymax = 100, trace = F, distances = "bray")
stressplot(eu.nmds)

pdf("output/Eukaryote/Composition_Analysis/Eusocial_NMDS_StressPlot.pdf")
  stressplot(eu.nmds)
dev.off()
...
#save
write.table(nmds.plot, "output/Eukaryote/Composition_Analysis/Eusocial_NMDS_meta.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)
...
#for eusocial animals, the most explanatory variables appear to be platform
#continent and year collected

write.table(eu.multiAOV, 
            "output/Eukaryote/Composition_Analysis/Eusocial_multiANOVA.tsv",
...
eu.disp.df
#everything is significantly dispersed
...
for (i in 1:nrow(eu.vars2)){
  ...
  ggsave(paste("output/Eukaryote/Composition_Analysis/Eusocial_NMDS_",
               title, ".pdf", sep = ""))
}
...
po.stress <- scree.NMDS(po.dist, "Eukaryote", "Polymorphic")
if(any(po.stress$Dec == "Excellent") == T){
  kval <- po.stress %>%
    subset(Dec == "Excellent") %>%
    arrange(NoDimensions) %>%
    head(n = 1) %>%
  ...
  print("Please assess by eye")
  stopifnot(any(po.stress$Dec == "Excellent"))
}
#script stopped because the lowest kval was 1
#manually going to put it as 2

po.nmds <- metaMDS(dist, k = 3, trymax = 100, trace = F, distances = "bray")

stressplot(po.nmds)
#not a great stress plot ... try 3

pdf("output/Eukaryote/Composition_Analysis/Polymorphic_NMDS_StressPlot.pdf")
  stressplot(po.nmds)
dev.off()
...
#save
write.table(nmds.plot, "output/Eukaryote/Composition_Analysis/Polymorphic_NMDS_meta.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)
...
#for polymorphic animals, nothing is significant

write.table(po.multiAOV, 
            "output/Eukaryote/Composition_Analysis/Polymorphic_multiANOVA.tsv",
            col.names = T, row.names = F, quote = F, sep = "\t")
...
po.disp.df %>%
  filter(Sig == "Non-Sig") %>%
  select(Variable)
#sex, host family and host tribe are non-significantly dispersed
for (i in 1:nrow(po.vars)){
  ...
  ggsave(paste("output/Eukaryote/Composition_Analysis/Polymorphic_NMDS_",
               title, ".pdf", sep = ""))
}
...
so.stress <- scree.NMDS(so.dist, "Eukaryote", "Solitary")
if(any(po.stress$Dec == "Excellent") == T){
  kval <- po.stress %>%
    subset(Dec == "Excellent") %>%
    arrange(NoDimensions) %>%
...
so.nmds <- metaMDS(dist, k = 3, trymax = 100, trace = F, distances = "bray")

stressplot(so.nmds)
#not a good stressplot so go back and use 3 dimensions

pdf("output/Eukaryote/Composition_Analysis/Solitary_NMDS_StressPlot.pdf")
  stressplot(so.nmds)
dev.off()
...
#save
write.table(so.plot, "output/Eukaryote/Composition_Analysis/Solitary_NMDS_meta.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)
...
#for solitary, nothing is significant

write.table(so.multiAOV, 
            "output/Eukaryote/Composition_Analysis/Solitary_multiANOVA.tsv",
            col.names = T, row.names = F, quote = F, sep = "\t")
...
so.disp.df %>%
  filter(Sig == "Non-Sig") %>%
  select(Variable)
#sex, year collected and host family/tribe are not considered significantly different
...
for (i in 1:nrow(so.vars)){
  ...
  ggsave(paste("output/Eukaryote/Composition_Analysis/Solitary_NMDS_",
               title, ".pdf", sep = ""))
}

##SessionLogs####
writeLines(capture.output(sessionInfo()),
           "SessionLogs/Analysis_Eukaryote_Jan23.txt")
