...
sum((as.vector(pca$CA$eig)/sum(pca$CA$eig))[1:3]) # 88.1%
...
write.table(multiSIM, 
            "output/Viral/Composition_Analysis/Multi_ANOSIM_testResults.tsv",
...
write.table(pairSIM.soc, 
            "output/Viral/Composition_Analysis/PairwiseANOSIM_Sociality.tsv",
            col.names = T, row.names = F, quote = F,
            sep = "\t")

#pairwise: Continent
#will have to remove south america as there's only one
cbn <- combn(x = unique(metdf$Continent[!metdf$Continent == "South America"]), m = 2)
pvalue <- c()
...
write.table(pairSIM.con, 
            "output/Viral/Composition_Analysis/PairwiseANOSIM_Continent.tsv",
            col.names = T, row.names = F, quote = F,
            sep = "\t")

#pairwise: Family
#will have to remove andrenidae ... though there's not much to look at here
#82 apidae versus 4 megachilidae
cbn <- combn(x = unique(metdf$Family[!metdf$Family == "Andrenidae"]),
             m = 2)
pvalue <- c()
...
eu.stress <- scree.NMDS(eu.dist, "Viral", "Eusocial")
...
pdf("output/Viral/Composition_Analysis/Eusocial_NMDS_StressPlot.pdf")
  stressplot(eu.nmds)
dev.off()
...
#save
write.table(nmds.plot, "output/Viral/Composition_Analysis/Eusocial_NMDS_meta.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)
...
#the only adjusted pvalues are sex and library layout (low r2)

write.table(eu.multiAOV, 
            "output/Viral/Composition_Analysis/Eusocial_multiANOVA.tsv",
...
for (i in 1:nrow(eu.vars2)){
  ...
  ggsave(paste("output/Viral/Composition_Analysis/Eusocial_NMDS_",
               title, ".pdf", sep = ""))
}
...
#so.stress <- scree.NMDS(so.dist, "Viral", "Solitary")
#if(any(so.stress$Dec == "Excellent") == T){
#  kval <- so.stress %>%
#    subset(Dec == "Excellent") %>%
#    arrange(NoDimensions) %>%
#    head(n = 1) %>%
...
#} else {
#  print("Please assess by eye")
#  stopifnot(any(so.stress$Dec == "Excellent"))
#}
# Warning about the number of samples ... it may not be enough to be informative (stress
# is very close to 0)
...
pdf("output/Viral/Composition_Analysis/Solitary_NMDS_StressPlot.pdf")
  stressplot(so.nmds)
dev.off()
...
#save
write.table(so.plot, "output/Viral/Composition_Analysis/Solitary_NMDS_meta.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)
...
write.table(so.multiAOV, 
            "output/Viral/Composition_Analysis/Solitary_multiANOVA.tsv",
            col.names = T, row.names = F, quote = F, sep = "\t")
...
for (i in 1:nrow(so.vars)){
  ...
  ggsave(paste("output/Viral/Composition_Analysis/Solitary_NMDS_",
               title, ".pdf", sep = ""))
}
